	...
	function __distruct()
	{
		...
		function view_apt_by_cit($cit_id){
         ...
         function view_objection(){
			 ...
			 $sql = sprintf("select * from objection_detail where isactive=1 order by obj_code "); 
		            $rs=mysql_query($sql,$this->link);
		            if(!$rs){
			         echo mysql_error($this->link);
			         }	
			...
			$sql = sprintf("select * from year_detail where isactive=1 order by year_id desc "); 
		            $rs=mysql_query($sql,$this->link);
		            if(!$rs){
			         echo mysql_error($this->link);
			         }
