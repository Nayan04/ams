                ...
                <td colspan="12" align="center"><strong>Summary CCIT Wise (RAP) </strong></td>
				  ...
				  $m=1;
							...
							while($audi_pp=mysql_fetch_array($audi_p))
							{
						       ...
						       $yes=$db->get_year_dist_by_ccit($audi_pp['id'],'RAP');	
							  ...
							  // $ci=$db->get_cit_by_id($cit);
							   ...
							   //$ccit=$cita['ccit_charge_id'];
							    if(mysql_num_rows($yes)>0)
							    {
							      ...
							      {			//echo $yrs['y'];										  
							        $reco=$db->get_registe_by_year_and_ccit($yrs['y'],$audi_pp['id'],'RAP'); //reg_obj tabel
							         $num_ob_by_year=0;
							         $num_ob_set_major=0;
							         $num_ob_set_minor=0;
							          $num_ob_rais_minor=0;
								    ...
								    $a_o=$db->get_cit_by_id($cit);
					                $oc=mysql_fetch_array($a_o);
								   echo "<tr><td>$m</td><td>".$audi_pp['ccit_charge']."</td><td></td><td>0</td><td>0</td><td>0</td><td>0</td><td>0</td><td>0</td><td>0</td></tr>";
								   $m++;
