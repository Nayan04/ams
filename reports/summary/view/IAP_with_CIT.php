<table id="example4" class="table table-bordered table-striped display" style="text-transform:capitalize;" >
				  ...
				  $m=1;
							...
							while($audi_pp=mysql_fetch_array($audi_p))
							{
						       ...
						       $yes=$db->get_year_dist_by_cit($audi_pp['id'],'IAP');	
							  ...
							  // $ci=$db->get_cit_by_id($cit);
							   ...
							   //$ccit=$cita['ccit_charge_id'];
							    if(mysql_num_rows($yes)>0)
							    {
							      ...
							      {			//echo $yrs['y'];										  
							        $reco=$db->get_registe_by_year_and_cit($yrs['y'],$audi_pp['id'],'IAP'); //reg_obj tabel
							         $num_ob_by_year=0;
							         $num_ob_set_major=0;
							         $num_ob_set_minor=0;
							          $num_ob_rais_minor=0;
								    ...
								    $a_o=$db->get_cit_by_id($cit);
					                $oc=mysql_fetch_array($a_o);
								   echo "<tr><td>$m</td><td>".$audi_pp['ccit_charge_name']."</td><td>".$asst['year']."</td><td>".$count."</td><td>".$num_ob_set_major."</td><td>".$num_ob_set_minor."</td><td>".$num_ob_rais_major."</td><td>".$num_ob_rais_minor."</td><td>".round(($tax_effect_major/1000000),2)."</td><td>".round(($tax_effect_minor/1000000),2)."</td></tr>";
								   $m++;
